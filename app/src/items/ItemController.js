(function () {
  ...
  angular
    ...
    .controller('ItemController', [
      ...
      '$rootScope', '$timeout', '$location', ItemController
    ]);
  /**
  ...
  function ItemController(itemService, $mdSidenav, $mdBottomSheet, $log, $q,
    ...
    function addTransitLayer() {
      ...
      if(!self.transitLayerOn) {
        ...
        self.transitLayer.setMap(self.instanceMap);
    ...
    function addTrafficLayer() {
      ...
      if(!self.trafficLayerOn) {
        ...
        self.trafficLayer.setMap(self.instanceMap);
    ...
    function loadInitialMap() {
      ...
      self.instanceMap = new google.maps.Map(
        document.getElementById('map_canvas'),
        {center: {
          lat: 40.72800630217342,
          lng: -73.99877832818606
        }, zoom: 14});
      google.maps.event.trigger(self.instanceMap, 'resize');

      updateBounds();

      // Updates search bounds on moving the map.
      google.maps.event.addListener(self.instanceMap, 'idle', updateBounds);

      itemService.loadAllItems().then(function (items) {
              self.items = items;
